...
repeat() {
	...
	if [ "$REPEAT" = "false" ]; then
		bottombar -m --set spotify.repeat icon.highlight=on
		osascript -e 'tell application "Spotify" to set repeating to true'
	else
		bottombar -m --set spotify.repeat icon.highlight=off
...
shuffle() {
	...
	if [ "$SHUFFLE" = "false" ]; then
		bottombar -m --set spotify.shuffle icon.highlight=on
		osascript -e 'tell application "Spotify" to set shuffling to true'
	else
		bottombar -m --set spotify.shuffle icon.highlight=off
...
scrubbing() {
	...
	bottombar --set spotify.state slider.percentage="$PERCENTAGE"
}
...
scroll() {
	...
	bottombar --animate linear 10 \
		--set spotify.state slider.percentage="$((TIME * 100 / DURATION))" \
		icon="$(/bin/date -r "$TIME" +'%M:%S')" \
		label="$(/bin/date -r $DURATION +'%M:%S')"
}
...
popup() {
	bottombar --set spotify.anchor popup.drawing="$1"
}
